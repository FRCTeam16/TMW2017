 ...
 *               The default is 50ms.
 */
BSPIDController::BSPIDController(double Kp, double Ki, double Kd, PIDSource* source,
                             PIDOutput* output, double period, double izone)
    : BSPIDController(Kp, Ki, Kd, 0.0, source, output, period, izone) {}
 ...
 */
void BSPIDController::Calculate() {
  bool enabled;
  PIDSource* pidInput;
  PIDOutput* pidOutput;
 ...
 * synchronization because the BSPIDController class only calls it in synchronized
 ...
 */
double BSPIDController::CalculateFeedForward() {
  if (m_pidInput->GetPIDSourceType() == PIDSourceType::kRate) {
 ...
 */
void BSPIDController::SetPID(double p, double i, double d) {
  {
    std::lock_guard<priority_recursive_mutex> sync(m_mutex);
    m_P = p;
    m_I = i;
 ...
 */
void BSPIDController::SetPID(double p, double i, double d, double f) {
  {
    std::lock_guard<priority_recursive_mutex> sync(m_mutex);
    m_P = p;
    m_I = i;
 ...
 */
double BSPIDController::GetP() const {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
  return m_P;
}
 ...
 */
double BSPIDController::GetI() const {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
  return m_I;
}
 ...
 */
double BSPIDController::GetD() const {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
  return m_D;
}
 ...
 */
double BSPIDController::GetF() const {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
  return m_F;
}
 ...
 */
double BSPIDController::Get() const {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
  return m_result;
}
 ...
 */
void BSPIDController::SetContinuous(bool continuous) {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
 ...
 */
void BSPIDController::SetInputRange(double minimumInput, double maximumInput) {
  {
    std::lock_guard<priority_recursive_mutex> sync(m_mutex);
 ...
 */
void BSPIDController::SetOutputRange(double minimumOutput, double maximumOutput) {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
...
/**
 * Set the setpoint for the BSPIDController.
 *
 * Clears the queue for GetAvgError().
 *
 * @param setpoint the desired setpoint
 */
void BSPIDController::SetSetpoint(double setpoint) {
  {
    std::lock_guard<priority_recursive_mutex> sync(m_mutex);
...
/**
 * Returns the current setpoint of the BSPIDController.
 *
 * @return the current setpoint
 */
double BSPIDController::GetSetpoint() const {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
  return m_setpoint;
}

/**
 * Returns the change in setpoint over time of the BSPIDController.
 *
 * @return the change in setpoint over time
 */
double BSPIDController::GetDeltaSetpoint() const {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
 ...
 */
double BSPIDController::GetError() const {
 ...
 */
void BSPIDController::SetPIDSourceType(PIDSourceType pidSource) {
 ...
 */
PIDSourceType BSPIDController::GetPIDSourceType() const {
 ...
 */
double BSPIDController::GetAvgError() const {
  double avgError = 0;
  {
    std::lock_guard<priority_recursive_mutex> sync(m_mutex);
 ...
 */
void BSPIDController::SetTolerance(double percent) {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
 ...
 * @param percentage error which is tolerable
 */
void BSPIDController::SetAbsoluteTolerance(double absTolerance) {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
 ...
 * @param percentage error which is tolerable
 */
void BSPIDController::SetPercentTolerance(double percent) {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
  m_toleranceType = kPercentTolerance;
  m_tolerance = percent;
}
 ...
 */
void BSPIDController::SetToleranceBuffer(int bufLength) {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
 ...
 */
bool BSPIDController::OnTarget() const {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
...
/**
 * Begin running the BSPIDController.
 */
void BSPIDController::Enable() {
  {
    std::lock_guard<priority_recursive_mutex> sync(m_mutex);
    m_enabled = true;
  }
...
/**
 * Stop running the BSPIDController, this sets the output to zero before stopping.
 */
void BSPIDController::Disable() {
  {
    std::lock_guard<priority_recursive_mutex> sync(m_mutex);
...
/**
 * Return true if BSPIDController is enabled.
 */
bool BSPIDController::IsEnabled() const {
  std::lock_guard<priority_recursive_mutex> sync(m_mutex);
  return m_enabled;
}
...
std::string BSPIDController::GetSmartDashboardType() const {
  return "BSPIDController";
}

void BSPIDController::InitTable(std::shared_ptr<ITable> subtable) {
  if (m_table != nullptr) m_table->RemoveTableListener(this);
  m_table = subtable;
  if (m_table != nullptr) {
 ...
 */
double BSPIDController::GetContinuousError(double error) const {
  if (m_continuous) {
...
std::shared_ptr<ITable> BSPIDController::GetTable() const { return m_table; }

void BSPIDController::ValueChanged(ITable* source, llvm::StringRef key,
...
void BSPIDController::UpdateTable() {}
